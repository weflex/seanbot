...
function issueCommentHandler(event) {
  ...
  ghGot(pullRequests.url, {token: token})
  .then((res) => {
    ...
    if (exec('git config --get user.email')) {
      exec('git config --global user.name "seanbot"');
      exec('git config --global user.email sean.lee@theweflex.com');
    }
